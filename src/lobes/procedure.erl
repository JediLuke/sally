...
%% @author Luke Taylor
%%
%% @doc The procedure is responsible for deciding what lobes are correct in their reasoning
%%
%%====================================================================
-module(procedure).
...
%%====================================================================
start_link(Params) ->
    %% io:format("procedure name: ~p~n",[Params]),
    %% Explicitely not assigning an ID to prevent 'already registered errors'
    %% as multiple procedures will spawn this module.
...
%%====================================================================
init([Params]) ->
    %% io:format("procedure: Initializing with ~p~n", [Params]),
...
handle_info({lobe, Inputs}, State) ->
    ...
    ebus:pub(State#state.name, {lobe_update, procedure, Results}),
